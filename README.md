## 🗺️ If You're Starting From Zero

Follow this order:

**1. Mathematics → 2. Programming → 3. Statistics → 4. Financial Markets → 5. Quant Finance → 6. Trading/Research → 7. ML → 8. Interviews**

Don't try to finish every resource. Pick **one good resource per topic and start building.**

---

## 📐 Mathematics & Probability

* **Introduction to Probability** — MIT OpenCourseWare
  https://ocw.mit.edu/resources/res-6-012-introduction-to-probability-spring-2018/

* **Topics in Mathematics with Applications in Finance** — MIT
  https://ocw.mit.edu/courses/mathematics/18-s096-topics-in-mathematics-with-applications-in-finance-fall-2013/

* **Numerical Linear Algebra for Coders** — fast.ai
  https://www.fast.ai/

### Books

* *Statistics* — Freedman, Pisani & Purves
* *Analysis of Financial Time Series* — Ruey S. Tsay
* *Thinking Strategically* — Dixit & Nalebuff

---

## 💻 Programming

### Python

* *Python for Finance* — Yves Hilpisch
* NumPy — https://numpy.org/
* pandas — https://pandas.pydata.org/

### C++

* *Accelerated C++* — Koenig & Moo
* *Effective Modern C++* — Scott Meyers
* C++ Reference — https://en.cppreference.com/

**For quant research:** Python first.
**For HFT / low latency:** add C++.

---

## 📊 Quant Finance

### Courses

* **Quantopian Lectures** — Python, statistics & trading
  https://gist.github.com/ih2502mk/50d8f7feb614c8676383431b056f4291

* **Introduction to Market Microstructure**
  https://www.institutlouisbachelier.org/mooc/practical-introduction-to-market-microstructure/

### Books

* *Options, Futures, and Other Derivatives* — John C. Hull
* *Advances in Financial Machine Learning* — Marcos López de Prado
* *Financial Calculus* — Martin Baxter

---

## 📈 Trading

These are the books I found particularly useful for developing my understanding of **entries, exits, market structure, trade selection and strategy development**:

* *Targeting Profitable Entry & Exit Points* — Alan Farley
* *The Master Swing Trader* — Alan Farley
* *The Four Cardinal Principles of Trading* — Bruce Babcock
* *Advanced Swing Trading* — John Crane
* *Dynamic Trading* — Robert Miner
* *High Probability Trading Strategies* — Robert Miner
* *Secrets for Profiting in Bull and Bear Markets* — Stan Weinstein
* *Swing Trading for Dummies*

> **Important:** Don't optimize for win rate alone. Evaluate **expectancy, drawdown, risk/reward, transaction costs and robustness**.

---

## 🤖 Machine Learning

### Courses

* **Machine Learning** — Tom Mitchell, CMU
  http://www.cs.cmu.edu/~tom/10701_sp11/

* **Introduction to Reinforcement Learning** — David Silver
  https://www.youtube.com/playlist?list=PLqYmG7hTraZDM-OYHWgPebj2MfCFzFObQ

### Books

* *Deep Learning* — Goodfellow, Bengio & Courville
* *Reinforcement Learning: An Introduction* — Sutton & Barto

---

## 📚 Research Papers

A few good papers to start with:

* **Machine Learning for Trading** — Gordon Ritter
  https://papers.ssrn.com/sol3/papers.cfm?abstract_id=3015609

* **Deep Hedging** — Hans Buehler et al.
  https://arxiv.org/pdf/1802.03042.pdf

* **A Stochastic Model for Order Book Dynamics** — Rama Cont et al.
  https://papers.ssrn.com/sol3/papers.cfm?abstract_id=1273160

* **Price Dynamics in a Markovian Limit Order Market** — Rama Cont et al.
  https://arxiv.org/pdf/1104.4596.pdf

---

## 🧠 Blogs & Research

* fast.ai — https://www.fast.ai/
* E. P. Chan — http://epchan.blogspot.com/
* Colah's Blog — http://colah.github.io/
* Distill — https://distill.pub/
* Sebastian Ruder — https://ruder.io/
* Systematic Edge — https://systematicedge.wordpress.com/
* Jonathan Kinlay — http://jonathankinlay.com/
* Applied Probability Notes — https://appliedprobability.blog/

---

## 🎯 Quant Interview Prep

Focus on:

**Probability → Statistics → Mathematics → Algorithms → Python/C++ → Markets**

### Recommended

* **Jane Street — Probability & Markets**
  https://www.janestreet.com/static/pdfs/trading-interview.pdf

---

## 🛠️ What To Build

Don't just consume resources.

Build:

* 📊 Backtesting engine
* 📈 Momentum / mean-reversion strategy
* 🔄 Pairs trading strategy
* 🧮 Options pricing model
* 📉 Volatility forecasting model
* 💰 Portfolio optimizer
* 📖 Limit order book simulator
* 🤖 ML-based trading strategy

### The loop

**Learn → Implement → Backtest → Validate → Improve**

---

## ⭐ Full Original Resource Collection

For the larger collection that inspired this repository:

**PyPatel — Quant-Finance-Resources**

https://github.com/PyPatel/Quant-Finance-Resources

---

### Disclaimer

For educational purposes only. Nothing here is financial advice or a recommendation to trade or invest.
